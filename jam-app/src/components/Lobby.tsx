...
function generateUniqueSessionKey(): string {
  ...
  let s1 = '';
  let s2 = '';
  for (let i = 0; i < 4; i++) s1 += chars.charAt(Math.floor(Math.random() * chars.length));
  for (let i = 0; i < 4; i++) s2 += chars.charAt(Math.floor(Math.random() * chars.length));
  return `${s1}-${s2}`;
}
...
export default function Lobby({ onJoin, isLoading = false, defaultRoomId }: LobbyProps) {
  ...
  const handleRegenerateKey = () => setCreatedRoomId(generateUniqueSessionKey());
          ...
          <button
            ...
            style={{
              ...
              background: mode === 'create'
                ...
                : 'linear-gradient(90deg, #00B0FF 0%, #7C4DFF 100%)',
            }}
          >
            {isLoading ? (
